...
import os
import time
import keyboard  # Librería para detectar Space
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            if len(landmarks) == 63:
                ...
                cv2.putText(frame, f"Texto: {current_text}", 
                            (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)

                # Guardar la letra solo si presionas Espacio
                if keyboard.is_pressed("space") and not captured:
                    captured = True  # Evitar múltiples capturas

                    if predicted_letter == "delete":
                        current_text = current_text[:-1]  # Borrar última letra
                    elif predicted_letter == "space":
                        current_text += " "  # Agregar espacio
                    elif predicted_letter == "nothing":
                        if current_text:
                            print(f"✅ Frase completada: {current_text}")
                            current_text = ""  # Limpiar para nueva frase
                    else:
                        current_text += predicted_letter
                        print(f"✅ Letra agregada: {predicted_letter}")

                if not keyboard.is_pressed("space"):
                    captured = False  # Permitir capturar de nuevo

    # Mostrar la imagen
    cv2.imshow("Reconocimiento de Lenguaje de Señas - Frases (Espacio para guardar)", frame)
